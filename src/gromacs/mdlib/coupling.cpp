...
// Check whether we're doing sinusoidal pressure
bool doSinP(const t_inputrec* ir)
{
   if (ir->sinp != eannNO)
   {
       return true;
...
// To initialize sinusoidal pressure
bool initSinP(t_inputrec* ir, gmx::Update* upd)
{
    bool doSinusoidalP = doSinP(ir);
    if (doSinusoidalP)
    {
       update_sinP_target_press(ir, ir->init_t, upd);
    }
    return doSinusoidalP;
}

/*Write function to determine whether or not the time == sinp_update time where we want to update the ref_p again*/
bool sinPtimeisDivisible(int value, int divisor) {
...
/*set target pressures if sinP enabled*/
void update_sinP_target_press(t_inputrec* ir, real t, gmx::Update* upd)
{
   /*Declare type of variables and initialize matrices*/
   int j, sinusoid_nsteps, update_period;
   matrix amplitude, tau, offset; 
   real insidesinP;
   matrix sinPtarget;
   
   sinusoid_nsteps=ir->sinp_nsteps;
   update_period = ir->sinp_update_period;
   
   /*initialize empty matrix to store sinP target values*/
   clear_mat(sinPtarget);
   j = 0;
   while (j < sinusoid_nsteps)
   {
      j++;
   }
   bool divisible = sinPtimeisDivisible(t,update_period);
   if (divisible)
   {
      for ( int d=0; d < ZZ; d++ )
      {
         insidesinP = (2 * M_PI * t)/ir->sinp_tau[d][d]; 
         sinPtarget[d][d] = ir->sinp_offset[d][d] + ir->sinp_amplitude[d][d]*std::sin(insidesinP); 
         ir->ref_p[d][d] = sinPtarget[d][d];
      }
      insidesinP = (2 * M_PI * t)/ir->sinp_tau[ZZ][ZZ]; 
      sinPtarget[ZZ][ZZ] = ir->sinp_offset[ZZ][ZZ] + ir->sinp_amplitude[ZZ][ZZ]*std::sin(insidesinP);
      ir->ref_p[ZZ][ZZ] = sinPtarget[ZZ][ZZ];
   }
   else
   {
      /*Don't want anything to happen (keep reference pressure as is)*/
   }     
}
