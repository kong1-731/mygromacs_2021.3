...
static void do_inputrec(gmx::ISerializer* serializer, t_inputrec* ir, int file_version)
{
    ...
    serializer->doBool(&ir->sinp);
    if (ir->sinp)
    {
        ir->sinp = TRUE;
    } 
    serializer->doInt(&ir->sinp_update_period);
    serializer->doRvec(&ir->sinp_amplitude[XX]);
    serializer->doRvec(&ir->sinp_amplitude[YY]);
    serializer->doRvec(&ir->sinp_amplitude[ZZ]);
    serializer->doRvec(&ir->sinp_tau[XX]);
    serializer->doRvec(&ir->sinp_tau[YY]);
    serializer->doRvec(&ir->sinp_tau[ZZ]);
    serializer->doRvec(&ir->sinp_offset[XX]);
    serializer->doRvec(&ir->sinp_offset[YY]);
    serializer->doRvec(&ir->sinp_offset[ZZ]);
    serializer->doInt(&ir->sinp_nsteps);
