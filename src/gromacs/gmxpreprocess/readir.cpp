            ...
            warninp_t       wi)
{
    ...
    snew(dumstr[3], STRLEN);
    snew(dumstr[4], STRLEN);
    ...
    printStringNewline(&inp, "SINUSOIDAL PRESSURE");
    ir->sinp = (get_eeenum(&inp, "sinp", yesno_names, wi) != 0);
    ir->sinp_update_period  = get_eint(&inp, "sinp_update_period", 0, wi);
    setStringEntry(&inp, "sinp_amplitude", dumstr[2], nullptr);
    setStringEntry(&inp, "sinp_tau", dumstr[3], nullptr);
    setStringEntry(&inp, "sinp_offset", dumstr[4], nullptr);
    ir->sinp_nsteps  = get_eint(&inp, "sinp_nsteps", 0, wi);
...
#undef CTYPE
    ...
    clear_mat(ir->sinp_amplitude);
    clear_mat(ir->sinp_tau);
    clear_mat(ir->sinp_offset);
    for (i = 0; i < DIM; i++)
    {
        ir->ref_p[i][i]    = dumdub[1][i];
        ir->compress[i][i] = dumdub[0][i];
        ir->sinp_amplitude[i][i] = dumdub[2][i];
        ir->sinp_tau[i][i] = dumdub[3][i];
        ir->sinp_offset[i][i] = dumdub[4][i];
    }
    if (ir->epct == epctANISOTROPIC)
    {
        ...
        ir->sinp_amplitude[XX][YY] = dumdub[2][3];
        ir->sinp_amplitude[XX][ZZ] = dumdub[2][4];
        ir->sinp_amplitude[YY][ZZ] = dumdub[2][5];
        ir->sinp_tau[XX][YY] = dumdub[3][3];
        ir->sinp_tau[XX][ZZ] = dumdub[3][4];
        ir->sinp_tau[YY][ZZ] = dumdub[3][5];
        ir->sinp_offset[XX][YY] = dumdub[4][3];
        ir->sinp_offset[XX][ZZ] = dumdub[4][4];
        ir->sinp_offset[YY][ZZ] = dumdub[4][5];
        for (i = 0; i < DIM; i++)
        {
            for (m = 0; m < i; m++)
            {
                ...
                ir->sinp_amplitude[i][m] = ir->sinp_amplitude[m][i];
                ir->sinp_tau[i][m] = ir->sinp_tau[m][i];
                ir->sinp_offset[i][m] = ir->sinp_offset[m][i];
            }
        }
    }
